...
knitr::opts_chunk$set(
  ...
  eval = has_targets  # Only evaluate if targets available
)

# Exit early if targets not available
if (!has_targets) {
  knitr::knit_exit()
}
...
**Note on Data Loading**: The visualization objects in this vignette are loaded using `readRDS()` rather than `tar_read()` because they're generated from pipeline metadata (`tar_meta()`, `tar_manifest()`) which would create circular dependencies if included in `_targets.R`. These objects are created by the `generate_visualizations.R` script after the main pipeline completes.
...
plot_build_times <- readRDS("../_targets/objects/plot_build_times")
...
time_stats <- readRDS("../_targets/objects/table_time_stats")
knitr::kable(
  time_stats,
...
plot_memory <- readRDS("../_targets/objects/plot_memory_usage")
print(plot_memory)
```
...
memory_stats <- readRDS("../_targets/objects/table_memory_stats")
knitr::kable(
  memory_stats,
...
manifest <- readRDS("../_targets/objects/table_manifest")
knitr::kable(
  manifest,
